  ...
  useEffect(() => {
    ...
    const fetchAllProducts = async () => {
      ...
      try {
        ...
        console.log("🔍 All Products API Response:", res.data);
        if (!cancelled) {
       setAllProducts(Array.isArray(res.data) ? res.data : res.data.data || []);
  setFilteredProducts(Array.isArray(res.data) ? res.data : res.data.data || []);
        }
      } catch (e) {
        if (!cancelled) {
          setAllProducts([]);
            ...
            return (
              <article
                ...
                className={`relative w-full rounded border ${
                  isSelected ? 'border-[#c81a1f] border-2' : 'border-[#D3D1D1]'
                } bg-white text-white shadow-sm p-3 md:p-3 lg:p-4 flex flex-col gap-4 cursor-pointer transition-all duration-200 ${
                  isSelected ? 'ring-2 ring-[#c81a1f]' : ''
                }`}
